...
import { loadAgentFilesystem } from './agentFilesystem.js'
import { loadAgentStateFilesystem } from './agentStateFilesystem.js'

const CORE_AGENT_IDS = ['nettie', 'van', 'perry', 'dana', 'torina', 'icky', 'funboy', 'rab']
...
export function buildAgentRegistryView({
  ...
  const workspace = loadAgentFilesystem(agentsRoot, { includeText: false })
  const stateWorkspace = loadAgentStateFilesystem(agentStateRoot)
  return registry.map((record) => {
    ...
    const workspaceRecord = workspace.byId[record.id] || null
    const stateRecord = stateWorkspace.byId[record.id] || null
    next.activeQueueCount = activeQueueCount
    next.lastSeenAt = lastSeenAt
    next.updatedAt = nowIso
    next.agentFilesystem = workspaceRecord ? {
      rootPath: workspaceRecord.rootPath,
      complete: workspaceRecord.complete,
      files: workspaceRecord.files,
      missingRequiredFiles: workspaceRecord.missingRequiredFiles,
      emptyFiles: workspaceRecord.emptyFiles,
      staleFiles: workspaceRecord.staleFiles,
      surfaces: workspaceRecord.surfaces,
    } : null
    next.agentState = stateRecord ? {
      rootPath: stateRecord.rootPath,
      complete: stateRecord.complete,
      files: stateRecord.files,
      missingRequiredFiles: stateRecord.missingRequiredFiles,
      invalidJsonFiles: stateRecord.invalidJsonFiles,
      staleFiles: stateRecord.staleFiles,
      summaries: stateRecord.summaries,
    } : null
    if (record.id === 'nettie') {
