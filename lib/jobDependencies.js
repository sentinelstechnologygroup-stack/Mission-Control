...
export function buildDependencyGraph({ jobs = [], now = new Date().toISOString() } = {}) {
  ...
  const nodes = deduped.map((job) => {
    ...
    return jobRow(job, {
      blockedBy: blockers,
      dependencyStatus,
      dependencyReason: blockers.length ? `Waiting on ${blockers.join(', ')}` : (dependsOn.length ? 'All prerequisites satisfied.' : null),
  ...
  return {
    generatedAt: now,
    blocked: nodes.filter((node) => node.dependencyStatus === 'blocked_by_dependency'),
