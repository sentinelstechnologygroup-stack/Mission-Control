...
export function detectNettieIntent(message = '') {
  ...
  const ownerMap = { ivy: 'Funboy', funboy: 'Funboy', van: 'Van', perry: 'Perry', torina: 'Torina', dana: 'Dana', rab: 'Rab', nettie: 'Nettie', bea: 'Bea' }
  const explicitOwnerMatch = lower.match(/\b(?:have|ask|queue|route|send|make|inspect|run|check|verify|review|report|investigate|work on)\s+(van|perry|torina|dana|ivy|funboy|rab|bea)\b/)
  const ownerMatch = lower.match(/\b(van|perry|torina|dana|ivy|funboy|rab|nettie|bea)\b/)
  const owner = explicitOwnerMatch ? ownerMap[explicitOwnerMatch[1]] : (ownerMatch ? ownerMap[ownerMatch[1]] || null : null)
  ...
  if ((/^(have|ask|queue|route|send|make)\b/.test(lower) || /^nettie\b.*\b(have|ask|queue|route|send|make)\b/.test(lower)) && owner) return { intent: 'route_to_agent', owner }
