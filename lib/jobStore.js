...
const purgeIndexPath = path.join(runtimeDir, 'purge-archive-index.json')
...
function normalizeWorkflowStep(step = null, fallback = 'intake') {
  const value = String(step || fallback || '').trim()
  if (!value) return fallback
  return value
}

function buildWorkflowExecutionRecord(job = {}, overrides = {}) {
  const status = normalizeStatus(overrides.status || job.status)
  const routeStatus = overrides.routeStatus || job.routeStatus || status
  const task = String(overrides.task || job.task || job.title || 'Untitled mission').trim()
  const owner = overrides.owner || job.owner || job.agent || job.department || 'Unassigned'
  const department = overrides.department || job.department || job.agent || job.owner || 'Unassigned'
  const createdAt = overrides.createdAt || job.createdAt || job.timestamps?.created || nowIso()
  const updatedAt = overrides.updatedAt || job.updatedAt || job.timestamps?.updated || createdAt
  const completedAt = overrides.completedAt || job.completedAt || job.timestamps?.completed || (isClosedStatus(status) ? updatedAt : null)
  const trace = Array.isArray(overrides.executionTrace) ? overrides.executionTrace : (Array.isArray(job.executionTrace) ? job.executionTrace : [])
  const history = toHistoryArray(overrides.history || job.history || [])
  const evidence = overrides.evidence || job.outputPayload || job.evidence || null
  const blockers = Array.isArray(overrides.blockers) ? overrides.blockers : [
    ...(Array.isArray(job.blockedBy) ? job.blockedBy : []),
    job.lockReason || null,
    job.outageReason || null,
    job.dependencyReason || null,
  ].filter(Boolean)
  const currentStep = normalizeWorkflowStep(overrides.currentStep || job.workflowExecution?.currentStep || (status === 'running' ? 'execution' : status === 'blocked' ? 'blocker' : 'intake'))
  const logs = (Array.isArray(overrides.logs) ? overrides.logs : trace).slice(-12).map((entry) => ({
    at: entry.at || entry.timestamp || updatedAt,
    level: entry.level || (String(entry.status || '').toLowerCase().includes('fail') ? 'error' : 'info'),
    step: entry.step || currentStep,
    message: entry.message || entry.text || entry.summary || '',
    data: sanitizeHistoryData(entry.data ?? entry),
  }))
  return {
    executionId: overrides.executionId || job.workflowExecution?.executionId || `wf_${String(job.id || job.jobId || crypto.randomUUID()).replace(/^job_/, '')}`,
    ...
    updatedAt,
    completedAt,
    nextAction: overrides.nextAction || job.nextAction || job.workflow?.nextAction || null,
    evidence,
    blockers,
    logs,
    history,
    workflow: overrides.workflow || job.workflow || null,
...
function canonicalizeJob(rawJob = {}, options = {}) {
  ...
  const job = {
    ...
    dependsOn: Array.isArray(rawJob.dependsOn) ? rawJob.dependsOn : (Array.isArray(existing?.dependsOn) ? existing.dependsOn : []),
    blockedBy: Array.isArray(rawJob.blockedBy) ? rawJob.blockedBy : (Array.isArray(existing?.blockedBy) ? existing.blockedBy : []),
    dependencyStatus: rawJob.dependencyStatus ?? existing?.dependencyStatus ?? null,
    dependencyReason: rawJob.dependencyReason ?? existing?.dependencyReason ?? null,
    unlocks: Array.isArray(rawJob.unlocks) ? rawJob.unlocks : (Array.isArray(existing?.unlocks) ? existing.unlocks : []),
    unblockReady: rawJob.unblockReady ?? existing?.unblockReady ?? false,
    lockOwner: rawJob.lockOwner ?? existing?.lockOwner ?? null,
    lockSession: rawJob.lockSession ?? existing?.lockSession ?? null,
    lockExpiresAt: rawJob.lockExpiresAt ?? existing?.lockExpiresAt ?? null,
    lockReason: rawJob.lockReason ?? existing?.lockReason ?? null,
    workflow: rawJob.workflow ?? existing?.workflow ?? null,
    workflowExecution: buildWorkflowExecutionRecord({ ...existing, ...rawJob, id: resolvedId, jobId: resolvedId, task, title: task, department, agent: rawJob.agent || department, owner: rawJob.owner || department, status, sourceType, source, createdAt, updatedAt, heartbeatAt, routeStatus: rawJob.routeStatus || existing?.routeStatus || status, nextAction: rawJob.nextAction ?? existing?.nextAction ?? null, outputPayload: rawJob.outputPayload !== undefined ? rawJob.outputPayload : existing?.outputPayload ?? null, blockedBy: Array.isArray(rawJob.blockedBy) ? rawJob.blockedBy : (Array.isArray(existing?.blockedBy) ? existing.blockedBy : []), lockReason: rawJob.lockReason ?? existing?.lockReason ?? null, outageReason: rawJob.outageReason ?? existing?.outageReason ?? null, dependencyReason: rawJob.dependencyReason ?? existing?.dependencyReason ?? null, executionTrace: Array.isArray(rawJob.executionTrace) ? rawJob.executionTrace : (Array.isArray(existing?.executionTrace) ? existing.executionTrace : []), history }, rawJob.workflowExecution || existing?.workflowExecution || {}),
...
function readPurgeArchiveIndex() {
  try {
    if (fs.existsSync(purgeIndexPath)) {
      const parsed = JSON.parse(fs.readFileSync(purgeIndexPath, 'utf8'))
      return {
        purgedJobIds: Array.isArray(parsed?.purgedJobIds) ? parsed.purgedJobIds : [],
        purgedSourceRefs: Array.isArray(parsed?.purgedSourceRefs) ? parsed.purgedSourceRefs : [],
      }
    }
  } catch {
    // ignore load failure and treat as empty
  }
  return { purgedJobIds: [], purgedSourceRefs: [] }
}
...
function loadJobs({ legacyJobs = [], legacyStateJobs = [], projectLedgerEntries = [], legacyRecoveryEntries = [] } = {}) {
  ...
  const purgeIndex = readPurgeArchiveIndex()
  const purgedJobIds = new Set((purgeIndex.purgedJobIds || []).map((value) => String(value || '').trim()).filter(Boolean))
  const purgedSourceRefs = new Set((purgeIndex.purgedSourceRefs || []).map((value) => String(value || '').trim()).filter(Boolean))
  const filteredRecoveryEntries = (legacyRecoveryEntries || []).filter((entry) => {
    const jobId = String(entry?.jobId || entry?.id || '').trim()
    const sourceRef = String(entry?.source || entry?.sourceRef || '').trim()
    return !(purgedJobIds.has(jobId) || (sourceRef && purgedSourceRefs.has(sourceRef)))
  })
  const filteredProjectLedgerEntries = (projectLedgerEntries || []).filter((entry) => {
    const jobId = String(entry?.jobId || entry?.id || '').trim()
    const sourceRef = String(entry?.source || entry?.sourceRef || '').trim()
    return !(purgedJobIds.has(jobId) || (sourceRef && purgedSourceRefs.has(sourceRef)))
  })
  const seed = [
    ...
    ...legacyJobs,
    ...legacyStateJobs,
    ...filteredRecoveryEntries.map((entry) => ({
    ...
    ...filteredProjectLedgerEntries.map((entry) => ({
      ...entry,
...
function updateJob(id, patch = {}) {
  ...
  if (patch.dependsOn !== undefined) next.dependsOn = Array.isArray(patch.dependsOn) ? patch.dependsOn : next.dependsOn
  if (patch.blockedBy !== undefined) next.blockedBy = Array.isArray(patch.blockedBy) ? patch.blockedBy : next.blockedBy
  if (patch.dependencyStatus !== undefined) next.dependencyStatus = patch.dependencyStatus
  if (patch.dependencyReason !== undefined) next.dependencyReason = patch.dependencyReason
  if (patch.unlocks !== undefined) next.unlocks = Array.isArray(patch.unlocks) ? patch.unlocks : next.unlocks
  if (patch.unblockReady !== undefined) next.unblockReady = Boolean(patch.unblockReady)
  if (patch.lockOwner !== undefined) next.lockOwner = patch.lockOwner
  if (patch.lockSession !== undefined) next.lockSession = patch.lockSession
  if (patch.lockExpiresAt !== undefined) next.lockExpiresAt = patch.lockExpiresAt
  if (patch.lockReason !== undefined) next.lockReason = patch.lockReason
  if (patch.workflow !== undefined) next.workflow = patch.workflow
...
function normalizeRegistryJob(job = {}) {
  return {
    ...
    dependencyStatus: job.dependencyStatus ?? null,
    dependencyReason: job.dependencyReason ?? null,
    unlocks: job.unlocks ?? [],
    unblockReady: job.unblockReady ?? false,
...
function toMissionStateJob(job = {}) {
  return {
    ...
    blockedBy: Array.isArray(job.blockedBy) ? job.blockedBy : [],
    dependencyStatus: job.dependencyStatus || null,
    unblockReady: Boolean(job.unblockReady),
    lockOwner: job.lockOwner || null,
    lockSession: job.lockSession || null,
    lockExpiresAt: job.lockExpiresAt || null,
    lockReason: job.lockReason || null,
...
function deriveWorkflowExecutions() {
  return canonicalJobs.map((job) => job.workflowExecution || buildWorkflowExecutionRecord(job)).filter(Boolean)
}
