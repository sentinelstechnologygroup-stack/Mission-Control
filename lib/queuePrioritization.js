...
function executionDisposition(job = {}, recoveryMeta = null) {
  const status = normalizeStatus(job.status)
  const routeStatus = String(job.routeStatus || '').toLowerCase()
...
export function buildQueuePriorities({ registry = {}, recoveryReport = null, claudeValidated = false, now = new Date().toISOString() } = {}) {
  ...
  const priorities = jobs.map((job) => {
    ...
    const disposition = executionDisposition(job, recoveryMeta)
