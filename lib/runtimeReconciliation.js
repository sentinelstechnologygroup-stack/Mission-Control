...
export function buildRuntimeLocksSummary({ ledgerJobs = [], registry = {}, now = new Date().toISOString() } = {}) {
  const openLedgerJobs = (ledgerJobs || []).filter((job) => !['complete', 'completed', 'failed', 'cancelled', 'archived'].includes(normalizeStatus(job.status)))
  const openJobs = registryOpenJobs(registry)
  const allJobs = uniqueById([...openLedgerJobs, ...openJobs])
  const activeLocks = []
  const staleLocks = []
  const lockConflicts = []
