...
function buildCosts({ state = {}, workers = [], jobs = [], runtimeDir }) {
  ...
  const usageRows = workers.map((worker) => {
    ...
    return {
      provider: worker.executor === 'codex' ? 'openai' : state.system?.selectedExecutor || 'unknown',
      model: worker.executor === 'codex' ? 'gpt-5-codex' : state.system?.selectedExecutor || 'unknown',
      modelVersion: state.system?.codexVersion || state.system?.version || 'unknown',
      taskType: job.type || 'execution',
      inputTokens: null,
      outputTokens: null,
      totalTokens: null,
      estimatedCost: null,
      ...
      project: inferJobProject(job),
      notes: 'Exact token and cost telemetry not yet wired; execution event captured only.',
      confidence: 'unavailable',
    }
  })

  const costByDepartment = DEPARTMENTS.map((department) => ({
  ...
  const workTypes = ['build', 'QA', 'research', 'blogs', 'reports', 'image generation', 'code repair', 'deployment', 'analysis']
  const costByWorkType = workTypes.map((type) => ({ workType: type, estimatedCost: null, trackingStatus: 'not-tracked' }))

  return {
    summary: {
      activeProvider: state.system?.selectedExecutor || 'unknown',
      activeModel: workers[0]?.executor === 'codex' ? 'gpt-5-codex' : state.system?.selectedExecutor || 'unknown',
      activeModelVersion: state.system?.codexVersion || state.system?.version || 'unknown',
      currentSessionTokenEstimate: null,
      todayTotalTokenUse: null,
      weeklyTokenUse: null,
      monthlyTokenUse: null,
      estimatedCostToday: null,
      estimatedCostThisWeek: null,
      estimatedCostThisMonth: null,
      source: 'No native token meter is currently wired into Mission Control. Executor/runtime metadata is real; token/cost values are unavailable.',
      lastUpdated: nowIso(),
      confidence: 'partial',
    },
    usageRows,
    burnRate: {
      ...
      tokensPerHour: null,
      estimatedTimeUntilCap: null,
      highUseWarning: usageRows.length > 5 ? 'Execution volume elevated; batch non-urgent long-form work.' : 'No token meter available; treat costs as unverified.',
      recommendedPauseWindow: 'Batch research/report work during low-urgency windows once token telemetry is live.',
    },
    cooldown: {
      provider: state.system?.selectedExecutor || 'unknown',
      model: workers[0]?.executor === 'codex' ? 'gpt-5-codex' : state.system?.selectedExecutor || 'unknown',
      cooldownStatus: 'unknown',
      cooldownStarted: null,
      estimatedResetTime: null,
      knownLimitType: 'not-tracked',
      nextRecommendedModel: state.system?.fallbackExecutor || 'unknown',
      fallbackSafe: Boolean(state.system?.fallbackExecutor),
    },
