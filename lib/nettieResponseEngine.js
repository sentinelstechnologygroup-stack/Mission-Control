...
export async function buildNettieCommandResponse({
  ...
  } else if (detected.intent === 'general_chat' || detected.intent === 'unknown') {
    summary = detected.intent === 'general_chat'
      ? 'Nettie answered in assistant-first chat mode.'
      : 'Nettie stayed conversational for an unclassified message.'
    replyMarkdown = detected.intent === 'general_chat'
      ? `I’m Nettie, your Mission Control executive assistant. I help you think through work, organize priorities, route tasks to the right AI coworkers, track execution, and report back clearly.

If you want, I can also summarize blocking work, route a task to Van/Perry/Torina/Dana/Icky/Funboy/Rab, or inspect runtime status.`
      : `I’m Nettie, and I didn’t detect a routing command in that message.

I’ll keep this conversational instead of switching to triage formatting. Tell me what you want to work on, or say “have Van …” if you want me to route a task.`
    ...
    if (detected.intent === 'clarification_request') {
      ...
      replyMarkdown = `I can fix that, but I need the exact target.

Tell me the route, file, API, or packet you want changed, and I’ll route it cleanly.`
    } else {
      summary = 'Nettie stayed conversational for an unclassified message.'
      replyMarkdown = `I’m Nettie, and I didn’t detect a routing command in that message.
...
I’ll keep this conversational instead of switching to triage formatting. Tell me what you want to work on, or say “have Van …” if you want me to route a task.`
  ...
  const threadId = context.threadId || context.conversationId || `nettie:${operator}:primary`
  const userMessage = {
    id: `${threadId}:user`,
    role: 'user',
    from: operator,
  ...
  const assistantMessage = {
    ...
    text: replyMarkdown,
    ts: now,
    kind: detected.intent === 'clarification_request' ? 'clarify' : (createdJobs.length ? 'packet' : 'assistant'),
  }
  return {
